...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('stock_movements', function (Blueprint $t) {
            $t->string('operation_reference')->nullable()->index();
        });
        Schema::table('inventory_sessions', function (Blueprint $t) {
            $t->unsignedBigInteger('last_movement_id')->default(0);
        });
        ...
        Schema::create('asset_operations', function (Blueprint $t) {
            ...
            $t->string('type', 32);
            $t->foreignId('source_location_id')->nullable()->constrained('locations')->restrictOnDelete();
        ...
        DB::table('audit_logs')->where('auditable_type', User::class)->orderBy('id')->each(function ($log) {
        ...
        if (DB::getDriverName() === 'pgsql') {
            ...
            DB::unprepared("CREATE OR REPLACE FUNCTION reject_history_change() RETURNS trigger LANGUAGE plpgsql AS 'BEGIN RAISE EXCEPTION ''Historical records are immutable''; END;'");
    ...
    public function down(): void
    {
        if (DB::getDriverName() === 'pgsql') {
            foreach (['stock_movements', 'audit_logs', 'asset_operations'] as $table) {
                DB::statement("DROP TRIGGER IF EXISTS immutable_history ON {$table}");
            }
        ...
        DB::statement('DROP INDEX IF EXISTS stock_balance_without_lot');
        DB::statement('DROP INDEX IF EXISTS inventory_line_without_lot');
        Schema::table('inventory_lines', function (Blueprint $t) {
            $t->dropUnique('inventory_line_lot_unique');
            $t->dropConstrainedForeignId('stock_lot_id');
            $t->dropConstrainedForeignId('adjustment_id');
        });
