...
class ProductController extends Controller
{
    ...
    public function store(ProductRequest $r, AuditService $audit)
    {
        return DB::transaction(function () use ($r, $audit) {
            ...
            $audit->log($r, 'product.created', $product);
    ...
    {
        return $product->load(['category.parent', 'unit', 'supplier'])->loadSum('balances as stock_quantity', 'quantity')
            ->setAttribute('has_movements', StockMovement::where('product_id', $product->id)->exists());
    }

    public function lots(Product $product)
    {
        return $product->lots()->withSum('balances as quantity', 'quantity')->orderBy('expires_at')->paginate(50);
    }

    public function balances(Product $product)
    {
        return $product->balances()->with(['lot', 'location'])->orderBy('id')->paginate(50);
    }
    ...
    public function update(ProductRequest $r, Product $product, AuditService $audit)
    {
        return DB::transaction(function () use ($r, $product, $audit) {
            ...
            $product->update($data);
            $audit->log($r, 'product.updated', $product, $old);
    ...
    public function destroy(Request $r, Product $product, AuditService $audit)
    {
        return DB::transaction(function () use ($r, $product, $audit) {
            ...
            $old = $product->getAttributes();
            $product->delete();
            $audit->log($r, 'product.archived', $product, $old);
