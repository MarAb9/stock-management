...
@Component({
  ...
  template: `
    <mat-sidenav-container class="shell">
      <mat-sidenav #drawer id="main-navigation" mode="side" opened class="nav">
        <div class="brand"><span>CSL</span><div>Conseil Scientifique<small>Berkane · Stock</small></div></div>
        @for (item of items; track item.path) {
          <a mat-button [routerLink]="item.path" routerLinkActive="active"><mat-icon>{{ item.icon }}</mat-icon>{{ item.label }}</a>
        }
      </mat-sidenav>
      <mat-sidenav-content>
        <mat-toolbar class="topbar">
          <button mat-icon-button (click)="drawer.toggle()" aria-label="Afficher ou masquer la navigation" aria-controls="main-navigation" [attr.aria-expanded]="drawer.opened"><mat-icon>menu</mat-icon></button>
          <span class="spacer"></span><span class="user">{{ api.user()?.name }}</span>
          <button mat-button [disabled]="loggingOut()" (click)="logout()">{{ loggingOut() ? 'Déconnexion…' : 'Déconnexion' }}</button>
        </mat-toolbar>
...
export class ShellComponent {
  ...
  readonly items = [
    ...
    { path: '/produits', icon: 'inventory_2', label: 'Produits' },
    { path: '/mouvements', icon: 'swap_horiz', label: 'Mouvements' },
    { path: '/equipements', icon: 'desktop_windows', label: 'Équipements' },
    { path: '/inventaires', icon: 'fact_check', label: 'Inventaires' },
    { path: '/rapports', icon: 'summarize', label: 'Rapports' },
    { path: '/parametres', icon: 'settings', label: 'Référentiels' },
  ];

  logout() {
    if (this.loggingOut()) return;
    this.loggingOut.set(true);
    this.api.post('auth/logout', {}).pipe(finalize(() => this.loggingOut.set(false))).subscribe({
