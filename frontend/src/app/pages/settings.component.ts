...
@Component({
  ...
  template: `
    <section class="heading"><h1>Référentiels</h1><p>Catégories, unités, fournisseurs et emplacements.</p></section><mat-card>
      <div class="bar"><mat-form-field><mat-label>Type</mat-label><mat-select [(ngModel)]="type" (selectionChange)="changeType($event.value)"><mat-option value="categories">Catégories</mat-option><mat-option value="units">Unités</mat-option><mat-option value="locations">Emplacements</mat-option><mat-option value="suppliers">Fournisseurs</mat-option></mat-select></mat-form-field>
        <mat-form-field><mat-label>Nom</mat-label><input matInput [(ngModel)]="form.name"></mat-form-field>
        @if(type!=='suppliers'){<mat-form-field><mat-label>{{type==='units'?'Symbole':'Code'}}</mat-label><input matInput [(ngModel)]="form.code"></mat-form-field>}
        @if(hasParent()){<mat-form-field><mat-label>Parent</mat-label><mat-select [(ngModel)]="form.parent_id"><mat-option [value]="null">Aucun parent</mat-option>@for(item of rows();track item.id){<mat-option [value]="item.id">{{item.name}}</mat-option>}</mat-select></mat-form-field>}
        @if(type==='locations'){<mat-form-field class="wide"><mat-label>Notes</mat-label><input matInput [(ngModel)]="form.notes"></mat-form-field>}
        @if(type==='suppliers'){<mat-form-field><mat-label>Société</mat-label><input matInput [(ngModel)]="form.company"></mat-form-field><mat-form-field><mat-label>Contact</mat-label><input matInput [(ngModel)]="form.contact_name"></mat-form-field><mat-form-field><mat-label>Téléphone</mat-label><input matInput [(ngModel)]="form.phone"></mat-form-field><mat-form-field><mat-label>Email</mat-label><input matInput type="email" [(ngModel)]="form.email"></mat-form-field><mat-form-field><mat-label>ICE</mat-label><input matInput [(ngModel)]="form.ice"></mat-form-field><mat-form-field class="wide"><mat-label>Adresse</mat-label><input matInput [(ngModel)]="form.address"></mat-form-field><mat-form-field class="wide"><mat-label>Notes</mat-label><input matInput [(ngModel)]="form.notes"></mat-form-field>}
        <button mat-flat-button [disabled]="!canSave() || saving()" (click)="save()">{{saving()?'Ajout…':'Ajouter'}}</button>
      </div>
...
export class SettingsComponent {
  ...
  constructor() { this.resetForm(); this.load(); }
  hasParent() { return this.type === 'categories' || this.type === 'locations'; }
  changeType(type: ReferenceType) { this.type = type; this.resetForm(); this.load(); }
  load() { this.loaded.set(false); this.api.get<any>('settings/' + this.type).pipe(finalize(() => this.loaded.set(true))).subscribe(response => this.rows.set(response.data)); }
  canSave() { return !!this.form.name && (this.type === 'suppliers' || !!this.form.code); }
  ...
  save() {
    ...
    this.api.post('settings/' + this.type, body).pipe(finalize(() => this.saving.set(false))).subscribe(() => {
      this.resetForm();
  ...
  private resetForm() { this.form = { name: '', code: '', parent_id: null, company: '', contact_name: '', phone: '', email: '', address: '', ice: '', notes: '' }; }
}
